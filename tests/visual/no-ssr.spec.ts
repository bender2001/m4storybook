...
/**
 * Design-parity tests for the M3-tokenized No SSR slot.
 *
 * MUI's `<NoSsr />` is a deferred-render utility that mounts children
 * only on the client, optionally yielding to the browser before commit
 * via the `defer` flag. M3 has no equivalent, so we re-skin it as a
 * surface-aware deferred slot:
 *
 *   - five surface variants (text / filled / tonal / outlined / elevated)
 *   - three densities driving padding, gap, min-height, body-type role
 *   - full M3 7-token shape scale
 *   - tri-state strategy (mount / defer / idle) drives commit timing
 *   - pending fallback fades out + deferred children fade in on the M3
 *     `medium2` (300ms) duration with the `emphasized` easing
 *
 * Spec sources:
...
test.describe("No SSR - M3 design parity", () => {
  test("default no-ssr renders the M3 deferred shell (filled / shape-lg / md size)", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--default"));
    const root = page.locator("[data-component='no-ssr']").first();
    await expect(root).toBeVisible();
    await expect(root).toHaveAttribute("data-variant", "filled");
    await expect(root).toHaveAttribute("data-size", "md");
    await expect(root).toHaveAttribute("data-shape", "lg");
    await expect(root).toHaveAttribute("data-strategy", "mount");
  ...
  test("variants paint the M3 surface matrix", async ({ page }) => {
    await page.goto(storyUrl("utils-no-ssr--variants"));
    ...
    for (const [variant, color] of Object.entries(expected)) {
      const root = page
        .locator(`[data-component='no-ssr'][data-variant='${variant}']`)
        .first();
      await expect(root).toBeVisible();
  ...
  test("outlined variant paints a 1dp outline-variant border", async ({
    ...
    await page.goto(storyUrl("utils-no-ssr--variants"));
    const root = page
      .locator("[data-component='no-ssr'][data-variant='outlined']")
      .first();
    const styles = await root.evaluate((el) => {
      const cs = window.getComputedStyle(el);
  ...
  test("elevated variant lifts to elevation-1", async ({ page }) => {
    await page.goto(storyUrl("utils-no-ssr--variants"));
    const root = page
      .locator("[data-component='no-ssr'][data-variant='elevated']")
      .first();
  ...
  test("size scale: 48 / 64 / 80 px min-heights for sm / md / lg", async ({
    ...
    await page.goto(storyUrl("utils-no-ssr--sizes"));
    const heights = await Promise.all(
      ["sm", "md", "lg"].map(async (size) => {
        const root = page
          .locator(`[data-component='no-ssr'][data-size='${size}']`)
          .first();
        await expect(root).toBeVisible();
  ...
  test("header typography steps title-s / title-m / title-l for sm / md / lg", async ({
    ...
    for (const [size, typo] of Object.entries(expected)) {
      const header = page
        .locator(
          `[data-component='no-ssr'][data-size='${size}'] [data-slot='header']`,
        )
        .first();
  ...
  test("disabled deferred slot: aria-disabled, opacity 0.38, pointer-events:none", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--states"));
    const root = page
      .locator("[data-component='no-ssr'][data-disabled='true']")
      .first();
    await expect(root).toBeVisible();
  ...
  test("selected deferred slot paints secondary-container + aria-selected", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--states"));
    const root = page
      .locator("[data-component='no-ssr'][data-selected='true']")
      .first();
  ...
  test("error deferred slot paints error-container + aria-invalid", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--states"));
    const root = page
      .locator("[data-component='no-ssr'][data-error='true']")
      .first();
  ...
  test("shape scale renders the canonical M3 radii", async ({ page }) => {
    await page.goto(storyUrl("utils-no-ssr--shapes"));
    const expected: Record<string, string> = {
      none: "0px",
      xs: "4px",
      sm: "8px",
    ...
    for (const [shape, value] of Object.entries(expected)) {
      const root = page
        .locator(`[data-component='no-ssr'][data-shape='${shape}']`)
        .first();
    ...
    const fullPill = page
      .locator("[data-component='no-ssr'][data-shape='full']")
      .first();
  ...
  test("strategies expose mount / defer / idle on the data attribute", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--strategies"));
    for (const strategy of ["mount", "defer", "idle"] as const) {
      const root = page
        .locator(`[data-component='no-ssr'][data-strategy='${strategy}']`)
        .first();
      await expect(root).toBeVisible();
      // After the deferred strategy resolves, data-mounted flips to "true".
      await expect(root).toHaveAttribute("data-mounted", "true");
      // aria-busy clears once mounted.
      const busy = await root.getAttribute("aria-busy");
      expect(busy).toBeNull();
    }
  });

  test("deferred children commit and replace the fallback slot", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--fallback"));
    const root = page.locator("[data-component='no-ssr']").first();
    await expect(root).toHaveAttribute("data-mounted", "true");
    const body = root.locator("[data-slot='body']").first();
    await expect(body).toBeVisible();
    expect(await root.locator("[data-slot='fallback']").count()).toBe(0);
  });

  test("leading + trailing icon slots render alongside the label", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--slots"));
    const leading = page
      .locator("[data-component='no-ssr'] [data-slot='icon-leading']")
      .first();
    const trailing = page
      .locator("[data-component='no-ssr'] [data-slot='icon-trailing']")
      .first();
    await expect(leading).toBeVisible();
    await expect(trailing).toBeVisible();
    const label = page
      .locator("[data-component='no-ssr'] [data-slot='label']")
      .first();
  ...
  test("M3 motion: emphasized easing + medium2 (300ms) on container transitions", async ({
    ...
    await page.goto(
      storyUrl("utils-no-ssr--motion", "light", "no-preference"),
    );
    const root = page.locator("[data-component='no-ssr']").first();
    const styles = await root.evaluate((el) => {
      const cs = window.getComputedStyle(el);
  ...
  test("polymorphic `as=section` mounts as a labelled region landmark", async ({
    ...
    await page.goto(
      storyUrl("utils-no-ssr--accessibility", "light", "no-preference"),
    );
    const region = page
      .locator("[data-component='no-ssr'][aria-label='Polymorphic section']")
      .first();
  ...
  test("override `as=div` drops the region role", async ({ page }) => {
    ...
    const div = page.locator("[data-component='no-ssr']").nth(1);
  ...
  test("dark theme swaps filled deferred slot to dark surface", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--variants", "dark"));
    const root = page
      .locator("[data-component='no-ssr'][data-variant='filled']")
      .first();
    const bg = await root.evaluate(
      (el) => window.getComputedStyle(el).backgroundColor,
    );
  ...
  test("text variant renders transparent host so it can wrap external surfaces", async ({
    ...
    await page.goto(storyUrl("utils-no-ssr--variants"));
    const root = page
      .locator("[data-component='no-ssr'][data-variant='text']")
      .first();
    const styles = await root.evaluate((el) => {
      const cs = window.getComputedStyle(el);
      return { bg: cs.backgroundColor, color: cs.color };
  ...
  test("playground story renders a NoSsr with role=region and label", async ({
    page,
  }) => {
    await page.goto(storyUrl("utils-no-ssr--playground"));
    const root = page.locator("[data-component='no-ssr']").first();
    await expect(root).toBeVisible();
