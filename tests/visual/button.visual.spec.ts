...
test.describe("Button - visual baselines", () => {
  for (const theme of ["light", "dark"] as const) {
    test(`variants row - ${theme}`, async ({ page }) => {
      await page.goto(storyUrl("inputs-button--variants", theme));
    ...
    test(`sizes row - ${theme}`, async ({ page }) => {
      await page.goto(storyUrl("inputs-button--sizes", theme));
      await page.waitForLoadState("networkidle");
      const root = page.locator("#storybook-root");
      await expect(root).toHaveScreenshot(
        `button-sizes-${theme}.png`,
        SCREENSHOT_OPTS,
      );
    });

    test(`states row - ${theme}`, async ({ page }) => {
      await page.goto(storyUrl("inputs-button--states", theme));
      await page.waitForLoadState("networkidle");
      const root = page.locator("#storybook-root");
      await expect(root).toHaveScreenshot(
        `button-states-${theme}.png`,
        SCREENSHOT_OPTS,
      );
    });

    test(`with icons - ${theme}`, async ({ page }) => {
      await page.goto(storyUrl("inputs-button--with-icons", theme));
      await page.waitForLoadState("networkidle");
      const root = page.locator("#storybook-root");
      await expect(root).toHaveScreenshot(
        `button-with-icons-${theme}.png`,
        SCREENSHOT_OPTS,
      );
    });
  }
