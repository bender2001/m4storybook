import { expect, test } from "@playwright/test";

/**
 * Design-parity tests for the M3-tokenized Autocomplete. M3 has no
 * dedicated Autocomplete spec; the field re-uses the M3 Text Field
 * spec (https://m3.material.io/components/text-fields/specs) and the
 * popup re-uses the M3 Menu spec (https://m3.material.io/components/menus/specs).
 * Each assertion reads computed styles so the test fails the moment a
 * token drifts.
 */

const storyUrl = (
  id: string,
  theme: "light" | "dark" = "light",
  reducedMotion: "reduce" | "no-preference" = "reduce",
) =>
  `/iframe.html?id=${id}&viewMode=story&globals=theme:${theme};reducedMotion:${reducedMotion}`;

const LIGHT_PRIMARY = "rgb(103, 80, 164)";
const LIGHT_OUTLINE = "rgb(121, 116, 126)";
const LIGHT_SURFACE_CONTAINER_HIGHEST = "rgb(230, 224, 233)";
const LIGHT_SURFACE_CONTAINER = "rgb(243, 237, 247)";
const LIGHT_ON_SURFACE_VARIANT = "rgb(73, 69, 79)";
const TRANSPARENT = "rgba(0, 0, 0, 0)";

// M3 emphasized easing token (medium2 / 300ms).
const EASE_EMPHASIZED = "cubic-bezier(0.2, 0, 0, 1)";

test.describe("Autocomplete - M3 design parity", () => {
  test("outlined variant matches M3 Text Field spec (radius, height, border, motion)", async ({
    page,
  }) => {
    await page.goto(storyUrl("inputs-autocomplete--default"));
    const combobox = page.getByRole("combobox");
    await expect(combobox).toBeVisible();

    const field = combobox.locator("xpath=..");
    const styles = await field.evaluate((el) => {
      const cs = window.getComputedStyle(el);
      return {
        height: cs.height,
        radius: cs.borderTopLeftRadius,
        bg: cs.backgroundColor,
        borderWidth: cs.borderTopWidth,
        borderColor: cs.borderTopColor,
        transitionDuration: cs.transitionDuration,
        transitionTimingFunction: cs.transitionTimingFunction,
      };
    });

    // M3 default Text Field height = 56dp.
    expect(styles.height).toBe("56px");
    // M3 outlined Text Field uses shape-xs (4dp).
    expect(parseFloat(styles.radius)).toBeCloseTo(4, 0);
    // Outlined fill is transparent.
    expect(styles.bg).toBe(TRANSPARENT);
    // Outlined border = outline role at 1dp resting.
    expect(parseFloat(styles.borderWidth)).toBeGreaterThan(0);
    expect(styles.borderColor).toBe(LIGHT_OUTLINE);
    // M3 emphasized motion: 300ms / cubic-bezier(0.2,0,0,1).
    const firstDuration = styles.transitionDuration.split(",")[0].trim();
    expect(firstDuration).toBe("0.3s");
    expect(styles.transitionTimingFunction).toContain(EASE_EMPHASIZED);
  });

  test("filled variant uses surface-container-highest fill", async ({ page }) => {
    await page.goto(storyUrl("inputs-autocomplete--variants"));
    const filledCombobox = page.getByRole("combobox", { name: "Filled" });
    const field = filledCombobox.locator("xpath=..");
    const styles = await field.evaluate((el) => {
      const cs = window.getComputedStyle(el);
      return {
        bg: cs.backgroundColor,
        borderBottomWidth: cs.borderBottomWidth,
      };
    });
    expect(styles.bg).toBe(LIGHT_SURFACE_CONTAINER_HIGHEST);
    expect(parseFloat(styles.borderBottomWidth)).toBeGreaterThan(0);
  });

  test("size scale matches M3 density (sm=40, md=56, lg=72)", async ({ page }) => {
    await page.goto(storyUrl("inputs-autocomplete--sizes"));
    const sm = page.getByRole("combobox", { name: "Small (40dp)" });
    const md = page.getByRole("combobox", { name: "Medium (56dp)" });
    const lg = page.getByRole("combobox", { name: "Large (72dp)" });

    const heights = await Promise.all(
      [sm, md, lg].map((c) =>
        c
          .locator("xpath=..")
          .evaluate((el) => window.getComputedStyle(el).height),
      ),
    );
    expect(heights).toEqual(["40px", "56px", "72px"]);
  });

  test("disabled state suppresses interactions and dims the field", async ({
    page,
  }) => {
    await page.goto(storyUrl("inputs-autocomplete--states"));
    const disabledCombobox = page.getByRole("combobox", { name: "Disabled" });
    await expect(disabledCombobox).toBeDisabled();

    const opacity = await disabledCombobox
      .locator("xpath=..")
      .evaluate((el) => parseFloat(window.getComputedStyle(el).opacity));
    expect(opacity).toBeCloseTo(0.38, 2);
  });

  test("error state swaps the border color to the error role", async ({
    page,
  }) => {
    await page.goto(storyUrl("inputs-autocomplete--states"));
    const errorCombobox = page.getByRole("combobox", { name: "Error" });
    const borderColor = await errorCombobox
      .locator("xpath=..")
      .evaluate((el) => window.getComputedStyle(el).borderTopColor);
    // Light error role = #B3261E → rgb(179, 38, 30).
    expect(borderColor).toBe("rgb(179, 38, 30)");
  });

  test("hover paints the field state-layer at 0.08 opacity", async ({ page }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--variants", "light", "no-preference"),
    );
    const filledCombobox = page.getByRole("combobox", { name: "Filled" });
    const field = filledCombobox.locator("xpath=..");

    await page.evaluate(() => (document.activeElement as HTMLElement)?.blur());
    await field.hover();
    await page.waitForTimeout(260);
    const opacity = await field
      .locator("[data-state-layer]")
      .evaluate((el) => parseFloat(window.getComputedStyle(el).opacity));
    expect(opacity).toBeCloseTo(0.08, 2);
  });

  test("focus opens the listbox and floats the label", async ({ page }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await expect(combobox).toHaveAttribute("aria-expanded", "true");
    await expect(page.getByRole("listbox")).toBeVisible();

    // Wait for the label font-size transition (medium2 = 300ms) to settle.
    await page.waitForTimeout(360);
    const label = page.locator("label");
    const fontSize = await label.evaluate(
      (el) => window.getComputedStyle(el).fontSize,
    );
    // Floating label uses label-m (12px) for md size.
    expect(parseFloat(fontSize)).toBeLessThanOrEqual(12);
  });

  test("popup matches M3 Menu spec (surface-container fill, elevation 2)", async ({
    page,
  }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await page.waitForTimeout(260);
    const popup = page.locator("[data-popup]");
    await expect(popup).toBeVisible();

    const styles = await popup.evaluate((el) => {
      const cs = window.getComputedStyle(el);
      return {
        bg: cs.backgroundColor,
        radius: cs.borderTopLeftRadius,
        boxShadow: cs.boxShadow,
      };
    });
    expect(styles.bg).toBe(LIGHT_SURFACE_CONTAINER);
    expect(parseFloat(styles.radius)).toBeCloseTo(4, 0);
    // Elevation 2 = a non-trivial multi-stop shadow.
    expect(styles.boxShadow).not.toBe("none");
    expect(styles.boxShadow).toMatch(/rgba?\(/);
  });

  test("keyboard navigation: ArrowDown highlights options, Enter selects", async ({
    page,
  }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await page.keyboard.press("ArrowDown");
    await page.keyboard.press("ArrowDown");
    await page.keyboard.press("Enter");
    await expect(combobox).not.toHaveAttribute("aria-expanded", "true");
    const value = await combobox.inputValue();
    expect(value.length).toBeGreaterThan(0);
  });

  test("Escape closes the popup", async ({ page }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await expect(page.getByRole("listbox")).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(page.getByRole("listbox")).toHaveCount(0);
  });

  test("active option paints state-layer at 0.08", async ({ page }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await page.keyboard.press("ArrowDown");
    await page.waitForTimeout(120);

    const layer = page
      .locator('[role="option"]')
      .first()
      .locator("[data-option-state-layer]");
    const opacity = await layer.evaluate((el) =>
      parseFloat(window.getComputedStyle(el).opacity),
    );
    expect(opacity).toBeCloseTo(0.08, 2);
  });

  test("resting label color comes from on-surface-variant", async ({ page }) => {
    await page.goto(storyUrl("inputs-autocomplete--default"));
    const labelColor = await page
      .locator("label")
      .evaluate((el) => window.getComputedStyle(el).color);
    expect(labelColor).toBe(LIGHT_ON_SURFACE_VARIANT);
  });

  test("focus-visible draws a 2dp primary outline on the field", async ({
    page,
  }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "light", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await page.waitForTimeout(260);
    const field = combobox.locator("xpath=..");
    const styles = await field.evaluate((el) => {
      const cs = window.getComputedStyle(el);
      return {
        borderWidth: cs.borderTopWidth,
        borderColor: cs.borderTopColor,
      };
    });
    // focus-within bumps the outlined border to 2dp + primary color.
    expect(parseFloat(styles.borderWidth)).toBeGreaterThanOrEqual(2);
    expect(styles.borderColor).toBe(LIGHT_PRIMARY);
  });

  test("dark theme swaps role colors on the popup", async ({ page }) => {
    await page.goto(
      storyUrl("inputs-autocomplete--default", "dark", "no-preference"),
    );
    const combobox = page.getByRole("combobox");
    await combobox.focus();
    await page.waitForTimeout(260);
    const popup = page.locator("[data-popup]");
    const bg = await popup.evaluate(
      (el) => window.getComputedStyle(el).backgroundColor,
    );
    expect(bg).not.toBe(LIGHT_SURFACE_CONTAINER);
  });
});
