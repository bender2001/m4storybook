...
/**
 * DateRangePicker anatomy + token bindings.
 *
 * Spec references (re-skinned MUI X DateRangePicker onto M3):
 ...
 *
 * The two endpoint cursors are shared `layoutId` motion spans that
 * spring between days. Their shape morphs from `shape-xs` to the
 * selected `shape` token via motion/react springs — the same M3
 * Expressive selection morph used by Tabs / Stepper / Pagination /
 * Date Picker / Data Grid. Days strictly between the endpoints paint
 * a continuous `primary-container` range fill.
 *
...
export const anatomy = {
  ...
  /** "Select range" supporting line. */
  panelHeaderEyebrow: "text-label-l text-on-surface-variant",
  /** Headline range in the header. */
  panelHeadline: [
  ...
  dayGrid: [
    "grid grid-cols-7 gap-y-0",
    "px-3 pb-3",
  ].join(" "),
  /**
   * The wrapper around each day cell. Hosts the continuous
   * `primary-container` range fill so adjacent days look connected.
   */
  dayCellWrap: [
    "relative isolate flex items-center justify-center",
    "h-10",
  ].join(" "),
  /** Continuous range-fill underlay for in-range days. */
  rangeFill: [
    "pointer-events-none absolute inset-y-0 z-[0]",
    "bg-primary-container",
    "transition-opacity duration-short4 ease-standard",
  ].join(" "),
  /** Range-fill — paints from left edge to cell center (start day). */
  rangeFillStart: "left-1/2 right-0",
  /** Range-fill — paints from cell center to right edge (end day). */
  rangeFillEnd: "left-0 right-1/2",
  /** Range-fill — paints across the entire cell (in-range days). */
  rangeFillFull: "left-0 right-0",
  ...
  /** Endpoint paint (token-driven; sits beneath the cursor span). */
  dayCellEndpoint: "text-on-primary",
  /** In-range day label paint. */
  dayCellInRange: "text-on-primary-container",
  /** Day cell label (lives above the state layer + cursors). */
  dayCellLabel: "relative z-[2] text-body-l tabular-nums",
  /** Endpoint cursor (shape-morphing motion span). */
  dayCursor: [
    "pointer-events-none absolute inset-0 z-[1]",
    "bg-primary",
  ].join(" "),
...
/** Variant matrix. */
export const variantClasses: Record<DateRangePickerVariant, VariantStyles> = {
  filled: {
...
interface SizeBlock {
  ...
  daySize: string;
  /** Day-row wrapper height (matches day-cell height). */
  dayRowHeight: string;
 ...
 */
export const sizeClasses: Record<DateRangePickerSize, SizeBlock> = {
  sm: {
...
/**
 * Panel + endpoint shape mapping.
 *
 * The panel container takes the shape token directly. The day cells
 * always paint `shape-full` (circular) per M3 — only the endpoint
 * cursors morph from `shape-xs` to the shape token via `layoutId`.
 */
export const shapeClasses: Record<DateRangePickerShape, string> = {
